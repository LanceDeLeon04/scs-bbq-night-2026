...
import {
  ...
  CheckCircle2, PackageCheck, ArrowLeft, Keyboard, Camera,
} from 'lucide-react'
import { supabase } from '../lib/supabaseClient.js'

// Lazily imported — html5-qrcode touches the DOM/camera APIs, so it's only
// pulled in once the modal actually mounts, not on every page load.
...
const SCANNER_ELEMENT_ID = 'dispatch-qr-scanner'

/**
 * Full dispatch workflow, run inside a modal:
 *   Scan QR code (camera starts automatically) or switch to manual search
 *     -> look up the order
...
export default function DispatchModal({ orders, onClose, onDispatched }) {
  ...
  // Camera scanning starts automatically as soon as the modal opens, and
  // stops automatically if the admin switches to manual search or closes
  // the modal — no "start camera" button needed.
  useEffect(() => {
    if (!manualMode) {
      startScanner()
    } else {
  ...
  const startScanner = async () => {
    ...
    try {
      ...
      setScanning(false)
      setScannerError(
        'Could not access the camera. Check permissions, or search manually below.'
      )
    }
  }
  ...
  const runLookup = (raw) => {
    ...
    // Exact ticket number match wins outright — this is what a QR scan produces.
            ...
            <div className="space-y-2.5">
              <div className="relative">
                ...
                <input
                  ...
                  placeholder="Type name, ticket number, ID no., mobile, or email"
